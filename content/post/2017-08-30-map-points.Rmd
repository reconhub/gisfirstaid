...
image: "post/2017-08-30-map-points_files/figure-html/density_map_3-1.png"
slug: map-points
...
library(deldir)
vor <- deldir(xy_clust$lon, xy_clust$lat)

gg_vor <- geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2),
		       data = vor$dirsgs,
    		       linetype = 1,
		       color= "black")
...
To complete this, we can represent the number of cases in each Voronoi polyon;
this is best done using new features in the *ggforce* package (install them by
typing: `devtools::install_github("thomasp85/ggforce")`):

```{r voronoi_2}

library(ggforce)

heat_colors2 <- scale_fill_gradientn("Number of cases", colors = c("white", "gold", "red", "darkred"))

base + geom_voronoi_tile(data = xy_clust, aes(fill = number), alpha = .5) +
  	            heat_colors2 +
